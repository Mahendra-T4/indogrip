...
// import 'package:indogrip/features/client/presentation/widgets/client_fields.dart';

abstract class AddClientPanelBuilder extends State<AddClientPanel> {
  ...
  final TextEditingController pManagerMobileNumberController =
      TextEditingController();
  final TextEditingController pAlterMobileNumberController =
      TextEditingController();
  ...
  Widget get pAlternateMobileNumberField => Expanded(
    child: CustomOptionalField(
      spacing: 10,
      controller: pAlterMobileNumberController,
      validator: (value) {
        if (value!.isNotEmpty || RegExp(Regex.phone).hasMatch(value)) {
          if (value == pManagerMobileNumberController.text) {
            return "Alternate number cannot be same as Owner Mobile Number";
          }
        }
        return null;
